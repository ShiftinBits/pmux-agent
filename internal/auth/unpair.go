...
func deleteFromServer(identity *Identity, serverURL string, path string, client *http.Client, hmacSecret string) error {
	token, err := ExchangeToken(identity, serverURL, client, hmacSecret)
	if err != nil {
...
func DeletePairing(identity *Identity, serverURL string, client *http.Client, hmacSecret string) error {
	return deleteFromServer(identity, serverURL, "/auth/pairing", client, hmacSecret)
}
...
func DeleteDevice(identity *Identity, serverURL string, client *http.Client, hmacSecret string) error {
	return deleteFromServer(identity, serverURL, "/auth/device", client, hmacSecret)
}
